...
$query = mysqli_query($con,
    "SELECT x.*, x1.nama_barang, x2.nama_merek, x3.nama_kategori
     FROM barang_masuk x JOIN barang x1 ON x1.idbarang=x.barang_id
     LEFT JOIN merek x2 ON x2.idmerek=x1.merek_id
     LEFT JOIN kategori x3 ON x3.idkategori=x1.kategori_id
     WHERE x.tanggal='$now' ORDER BY x.idbarang_masuk DESC"
) or die(mysqli_error($con));

$totalJumlah = 0; $totalHarga = 0; $rows = [];
...
<title>Laporan Barang Masuk Hari Ini</title>
<style>
...
<div class="judul">
    ...
    <div class="sub">Tanggal: <?= date('d F Y') ?></div>
</div>
<table class="data">
    <thead><tr>
        <th width="25">NO</th><th width="80">TANGGAL</th><th>NAMA BARANG</th>
        <th width="75">MEREK</th><th width="75">KATEGORI</th><th>KETERANGAN</th>
        <th width="40">JML</th><th width="110">TOTAL HARGA</th>
    </tr></thead>
    <tbody>
    <?php if (empty($rows)): ?>
        <tr><td colspan="8" align="center" style="padding:12px;">Tidak ada barang masuk hari ini</td></tr>
    <?php else: $n=1; foreach ($rows as $row): ?>
        <tr>
            ...
            <td align="right"><?= ($row['harga']??0)>0 ? 'Rp '.number_format($row['harga'],0,',','.') : '-' ?></td>
        </tr>
    ...
    <tr class="row-total">
        ...
        <td align="right"><?= $totalHarga>0 ? 'Rp '.number_format($totalHarga,0,',','.') : '-' ?></td>
    </tr>
</table>
